	...
	},
	{
		...
		config = function()
				...
				callback = function(event)
					...
					mapn("<leader>tt", function()
						vim.diagnostic.config({
							virtual_text = not vim.diagnostic.config().virtual_text and virtual_text_on,
						})
					...
					mapn("<leader>tl", function()
						vim.diagnostic.config({
							virtual_lines = not vim.diagnostic.config().virtual_lines and virtual_lines_on,
						})
					...
					if
						client and client:supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight, 0)
					then
			...
			if vim.lsp.inline_completion then
				...
				vim.keymap.set("i", "<m-y>", vim.lsp.inline_completion.get, { desc = "LSP: Accept Inline Suggestion" })
				vim.keymap.set("i", "<m-]>", function()
					vim.lsp.inline_completion.select({ count = 1 })
				end, { desc = "LSP: Next Inline Suggestion" })
				vim.keymap.set("i", "<m-]>", function()
					vim.lsp.inline_completion.select({ count = -1 })
				end, { desc = "LSP: Previous Inline Suggestion" })
			end
