vim.keymap.set({ "n", "v" }, "<leader>gd", function()
	--- E.g. just a word, like `stg_foo__bar`
	...
	local files = vim.fs.find(url .. ".py", {})
	if not files then
		vim.lsp.buf.definition({})
		return
	end
	...
	if #files == 1 then
		...
		local layer
		for _, layer_type in ipairs({ "gold", "silver", "bronze", "stg", "int", "processed", "log" }) do
			layer = cword:match("tbl_(" .. layer_type .. ")")
			if layer then
				break
			end
		end
